...
class CalenderModel {
  ...
  constructor() {
    ...
    this.adjustmentCalenderUseCase = new AdjustmentCalenderUseCase(this.calenderRepository);
  }
  ...
  async saveCalender(
    ...
    try {
      await this.saveCalenderUseCase.execute(1, name, label, deadline, estimatedTime);
    } catch (error) {
      return Promise.reject(error);
    }
  }
